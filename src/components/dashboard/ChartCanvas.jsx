...
const valueLabels = {
  ...
  afterDatasetsDraw(chart, _a, opts) {
    ...
    ctx.fillStyle = opts.color || Chart.defaults.color || "#0f172a";
...
/**
 * ChartCanvas — thin wrapper over chart.js.
 * @param {"bar"|"line"|"doughnut"|"radar"} type
 * @param {object} data                chart.js data object
 * @param {object} options             chart.js options (merged over sane defaults)
 * @param {number} height              px
 * @param {(index:number, datasetIndex:number, label:string)=>void} onElementClick
 *        fired when a bar/point/arc is clicked — used for drill-down.
 */
export default function ChartCanvas({ type, data, options, height = 240, onElementClick }) {
  ...
  const baseOpts = () => ({
    ...
    onClick: (evt, els, chart) => {
      if (!clickRef.current || !els || !els.length) return;
      const { index, datasetIndex } = els[0];
      const label = chart?.data?.labels?.[index];
      clickRef.current(index, datasetIndex, label);
    },
    ...options,
  });
...
export function Gauge({ value, label, sub, color = PALETTE.blue, track, height = 176 }) {
  const pct = Math.max(0, Math.min(100, (value || 0) * 100));
  // Theme-aware empty-arc colour when the caller doesn't override it, so the
  // gauge track isn't a bright white ring in dark mode.
  const isDark =
    typeof document !== "undefined" &&
    document.documentElement.getAttribute("data-theme") === "dark";
  const trackColor = track || (isDark ? "#222c3c" : PALETTE.track);
  return (
    <div style={{ position: "relative", width: "100%", minWidth: 0, height }}>
      ...
      >
        <div>
          <div style={{ fontSize: 23, fontWeight: 800, color: "var(--hub-text, #0F172A)", lineHeight: 1 }}>
            {pct.toFixed(pct % 1 ? 1 : 0)}%
          </div>
          <div style={{ fontSize: 10.5, color: "var(--hub-muted, #64748b)", marginTop: 4, fontWeight: 600 }}>
            {label}
          </div>
          {sub != null && (
            <div style={{ fontSize: 10, color: "var(--hub-muted, #94a3b8)", marginTop: 2 }}>{sub}</div>
          )}
        </div>
      </div>
    </div>
