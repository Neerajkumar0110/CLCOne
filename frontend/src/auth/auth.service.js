...
export const verifyOtp = async ({ email, otp, remember }) => {
  try {
    const response = await axios.post(
      API_BASE_URL + `login/verify-otp?timestamp=${new Date().getTime()}`,
      { email, otp, remember }
    );
...
export const resendOtp = async ({ email }) => {
  try {
    const response = await axios.post(
      API_BASE_URL + `login/resend-otp?timestamp=${new Date().getTime()}`,
      { email }
    );
