...
export const login =
  ...
  async (dispatch) => {
    ...
    if (data.success === true && data.result?.otpRequired) {
      dispatch({
        type: actionTypes.OTP_REQUIRED,
...
export const loginWithPassword =
  ...
  async (dispatch) => {
    ...
    const data = await authService.loginWithPassword({ loginData });
...
export const verifyOtp =
  ...
  async (dispatch) => {
    ...
    const data = await authService.verifyOtp({ email, otp, remember });
...
export const cancelOtp = () => (dispatch) => {
